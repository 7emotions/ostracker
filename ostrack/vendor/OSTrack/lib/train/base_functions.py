...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "CUSTOM_GOT10K_train", "CUSTOM_GOT10K_val", "COCO17", "VID", "TRACKINGNET"]
        if name == "LASOT":
        ...
        if name == "CUSTOM_GOT10K_train":
            if settings.use_lmdb:
                raise ValueError("Custom GOT10K-like dataset does not support lmdb mode")
            else:
                datasets.append(Got10k(settings.env.got10k_dir, split=None, image_loader=image_loader))
        if name == "CUSTOM_GOT10K_val":
            if settings.use_lmdb:
                raise ValueError("Custom GOT10K-like dataset does not support lmdb mode")
            else:
                datasets.append(Got10k(settings.env.got10k_val_dir, split=None, image_loader=image_loader))
